                ...
                <b-card-text>
                  ...
                  >
                    ...
                    <template #cell(email)="row">
                      <input v-model="row.item.email" type="email" class="btn px-0">
                    </template>
                    <template #cell(cedula)="row">
                      <input v-model="row.item.cedula" type="number" class="btn px-0">
                    </template>
                    <template #cell(birthday)="row">
                      <input v-model="row.item.birthday" type="date" class="btn px-0">
                    </template>
      ...
      <script>
        let appAdmin = new Vue({
          ...
          data: {
            ...
            form: {
              inputs: [
                ...
                {label: 'Nacimiento', key: 'birthday', colSize: '6', placeholder: '', type: 'date', required: true},
                {label: 'Contrasena', key: 'password', colSize: '6', placeholder: '', type: 'password', required: true}
              ]
            },
            showSidebar: false,
            name: '',
          ...
          methods:{
            ...
            async createUser(){
              try {
                let response = await axios.post("{{route('users.store')}}", {data: this.newUser}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
                this.showAlert(response.data.message, 'success')
              } catch(error) {
                console.log(error.response.status)
