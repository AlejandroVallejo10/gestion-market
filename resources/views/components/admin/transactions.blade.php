                ...
                <b-card-text>
                  ...
                  >
                    <template #cell(total)="row">
                      <input v-model="row.item.total" type="number" class="btn px-0">
                    </template>
                    <template #cell(fecha)="row">
                      <input v-model="row.item.fecha" type="date" class="btn px-0">
                    </template>
                    <template #cell(user_id)="row">
                      <input v-model="row.item.user_id" type="number" class="btn px-0">
                    </template>
      ...
      <script>
        let appAdmin = new Vue({
          ...
          data: {
            ...
            form: {
              inputs: [
                {label: 'Total', key: 'total', colSize: '6', placeholder: '', type: 'number', required: true},
                {label: 'Fecha', key: 'fecha', colSize: '6', placeholder: '', type: 'date', required: true},
                {label: 'Usuario Id', key: 'user_id', colSize: '6', placeholder: '', type: 'number', required: true},
              ]
            },
            showSidebar: false,
            name: '',
            ...
            table: {
                ...
                fields: [
                    ...
                    {'key' : 'user_id', 'label' : 'USUARIO ID', 'sortable' : true},
          ...
          methods:{
            async updateUser(userObj){
              ...
              try {
                let response = await axios.put("{{route('transactions.index')}}/" + userObj.id, {data: copyUserData}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
            ...
            async deleteUser(userId){
              try {
                let response = await axios.delete("{{route('transactions.index')}}/" + userId, {}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
                this.showAlert(response.data.message, 'success')
              } catch(error) {
                console.log(error.response.status)
            ...
            async createUser(){
              try {
                let response = await axios.post("{{route('transactions.store')}}", {data: this.newUser}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
                this.showAlert(response.data.message, 'success')
              } catch(error) {
                console.log(error.response.status)
            ...
            async getUsers(){
              let response = await axios.get("{{route('transactions.index')}}", {}, {headers:{'Content-type': 'application/json'}})
