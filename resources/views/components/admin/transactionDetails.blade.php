                ...
                <b-card-text>
                  ...
                  >
                    <template #cell(quantity)="row">
                      <input v-model="row.item.quantity" type="number" class="btn px-0">
                    </template>
                    <template #cell(transaction_id)="row">
                      <input v-model="row.item.transaction_id" type="number" class="btn px-0">
                    </template>
                    <template #cell(product_id)="row">
                      <input v-model="row.item.product_id" type="number" class="btn px-0">
                    </template>
      ...
      <script>
        let appAdmin = new Vue({
          ...
          data: {
            ...
            form: {
              inputs: [
                {label: 'Cantidad', key: 'quantity', colSize: '6', placeholder: '', type: 'number', required: true},
                {label: 'Venta id', key: 'transaction_id', colSize: '6', placeholder: '', type: 'number', required: true},
                {label: 'Producto id', key: 'product_id', colSize: '6', placeholder: '', type: 'number', required: true},
              ]
            },
            showSidebar: false,
            name: '',
            ...
            table: {
                ...
                fields: [
                    ...
                    {'key' : 'transaction_id', 'label' : 'VENTA ID', 'sortable' : true},
                    {'key' : 'product_id', 'label' : 'PRODUCTO ID', 'sortable' : true},
          ...
          methods:{
            async updateUser(userObj){
              ...
              try {
                let response = await axios.put("{{route('transactionDetails.index')}}/" + userObj.id, {data: copyUserData}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
            ...
            async deleteUser(userId){
              try {
                let response = await axios.delete("{{route('transactionDetails.index')}}/" + userId, {}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
                this.showAlert(response.data.message, 'success')
              } catch(error) {
                console.log(error.response.status)
            ...
            async createUser(){
              try {
                let response = await axios.post("{{route('transactionDetails.store')}}", {data: this.newUser}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
                this.showAlert(response.data.message, 'success')
              } catch(error) {
                console.log(error.response.status)
            ...
            async getUsers(){
              let response = await axios.get("{{route('transactionDetails.index')}}", {}, {headers:{'Content-type': 'application/json'}})
