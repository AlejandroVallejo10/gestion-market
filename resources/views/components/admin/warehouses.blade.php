                ...
                <b-card-text>
                  ...
                  >
                    ...
                    <template #cell(store_id)="row">
                      <input v-model="row.item.store_id" type="number" class="btn px-0">
                    </template>
        ...
        let appAdmin = new Vue({
          ...
          data: {
            ...
            form: {
              inputs: [
                ...
                {label: 'Tienda id', key: 'store_id', colSize: '6', placeholder: '', type: 'number', required: true},
            ...
            table: {
                ...
                fields: [
                    ...
                    {'key' : 'store_id', 'label' : 'TIENDA ID', 'sortable' : true},
          ...
          methods:{
            async updateUser(userObj){
              ...
              try {
                let response = await axios.put("{{route('warehouses.index')}}/" + userObj.id, {data: copyUserData}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
            ...
            async deleteUser(userId){
              try {
                let response = await axios.delete("{{route('warehouses.index')}}/" + userId, {}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
                this.showAlert(response.data.message, 'success')
              } catch(error) {
                console.log(error.response.status)
            ...
            async createUser(){
              try {
                let response = await axios.post("{{route('warehouses.store')}}", {data: this.newUser}, {headers:{'Content-type': 'application/json'}})
                this.getUsers()
                this.showAlert(response.data.message, 'success')
              } catch(error) {
                console.log(error.response.status)
            ...
            async getUsers(){
              let response = await axios.get("{{route('warehouses.index')}}", {}, {headers:{'Content-type': 'application/json'}})
